defmodule CercleApi.APIV2.BulkControllerTest do
  ...
  test "create bulk contact with contacts more than 100 records", %{conn: conn} = state do
    ...
    conn = post conn, "/api/v2/company/#{state[:company].id}/bulk_contact_create", items: items, return: false
    assert json_response(conn, 200)["error_message"] == "Maximum 100 records are permitted per call"
  end

  test "create bulk contact with contacts less than 100", %{conn: conn} = state  do
  ...
  test "create bulk_tag_or_untag_contacts with more than 100 records", %{conn: conn} = state  do
    tag_id = "1"
    untag = false
    contacts = for i <- 0..150 do
      Integer.to_string i
    end
    conn = post conn, "/api/v2/company/#{state[:company].id}/bulk_tag_or_untag_contacts", contacts: contacts, tag_id: tag_id, untag: untag, return: false
    assert json_response(conn, 200)["error_message"] == "Maximum 100 records are permitted per call"
  end

  test "create bulk_tag_or_untag_contacts with less than 100 records but params missing", %{conn: conn} = state  do
    contacts = ["1"]
    conn = post conn, "/api/v2/company/#{state[:company].id}/bulk_tag_or_untag_contacts", contacts: contacts, tag_id: nil , untag: nil, return: false
    assert json_response(conn, 200)["responses"]["errors"] == "Parameters missing"
  end

  test "create bulk_tag_or_untag_contacts with invalid tag_id", %{conn: conn} = state   do
    untag = false
    tag_id = "1"
    contacts = ["1"]
    conn = post conn, "/api/v2/company/#{state[:company].id}/bulk_tag_or_untag_contacts", contacts: contacts, tag_id: tag_id, untag: untag, return: false
  ...
  test "create bulk_tag_or_untag_contacts with invalid contact_id", state  do
    ...
    untag = false
    contacts = ["1111"]
    conn = post state[:conn], "/api/v2/company/#{state[:company].id}/bulk_tag_or_untag_contacts", contacts: contacts, tag_id: str_tag_id, untag: untag, return: false
